...
const findAvailablePengelola = async (excludeNumbers = [], kategori) => {
  try {
...
// === HELPER FUNCTION: CEK APAKAH KATEGORI PUNYA MULTIPLE PENGELOLA ===
const hasMultiplePengelola = async (kategori) => {
  try {
    const { getAllPengelolaNumbers } = require('../../db');
    const allPengelola = await getAllPengelolaNumbers(kategori);
...
const setStateBekasan = async (chatId, state) => {
  const { nomor_hp, userId, kategori, attemptedPengelola = [] } = state;
  ...
  if (activePengelolaSessions.has(nomor_hp)) {
    // === CEK APAKAH KATEGORI PUNYA MULTIPLE PENGELOLA ===
    const hasMultiple = await hasMultiplePengelola(kategori);
    
    if (hasMultiple) {
      // === AUTO SWITCH LOGIC (untuk kategori dengan multiple pengelola) ===
      // Cari nomor pengelola lain yang kosong
      const newAttemptedList = [...attemptedPengelola, nomor_hp];
      const alternatePengelola = await findAvailablePengelola(newAttemptedList, kategori);
...
module.exports = (bot) => {
  ...
  bot.on('message', async (msg) => {
    ...
    const { nomor_slot, kategori } = state;
    
    // Untuk bekasan, kuota selalu 0GB (hardcoded)
    const kuotaGB = '0';
      ...
      if (validationResult.valid === false) {
        ...
        try {
          ...
          await logTransaction(bot, {
            ...
            kategori: state.kategori.toUpperCase(), // Gunakan state.kategori
    ...
    try {
      ...
      formData.append('child_name', `${msg.from.username || msg.from.first_name || 'USER'} BEKASAN ${kategori.toUpperCase()}`); // ✅ Sesuai ADD1.js
      ...
      if (addExecutionTime >= 8) {
        ...
        await freezeStok(kategori.toUpperCase(), nomor_hp, normalizedNumber, nomor_slot, kuotaGB, msg.from.id, bot, {
      ...
      setTimeout(async () => {
        ...
        if (addExecutionTime >= 8) {
          setTimeout(async () => {
            // ✅ DECLARE VARIABLES OUTSIDE TRY-CATCH untuk menghindari ReferenceError
            let kuberInBytes;
            ...
            try {
              ...
              if (recheckRes.data?.data?.member_info?.members) {
                ...
                // ✅ TAMBAH ADDITIONAL_MEMBERS jika ada (seperti handler_bulanan.js)
                ...
                // console.log(`📋 Total members (including additional): ${freshMemberList.length}`);
              ...
              if (freshMemberList.length > 0) {
                ...
                // console.log(`🔍 Looking for member with msisdn: ${targetMsisdn} or ${normalizedNumber}`);
                ...
                // ✅ CARI DI MEMBER BIASA DULU
                let targetMember = membersOnly.find(member => 
                  member.msisdn === targetMsisdn || member.msisdn === normalizedNumber
                );
                
                if (targetMember) {
                  familyMemberId = targetMember.family_member_id;
                  // console.log(`✅ Found target member at index ${freshMemberList.indexOf(targetMember)}: ${targetMember.msisdn}`);
                } else {
                  // ✅ FALLBACK: Gunakan family_member_id dari ADD response jika ada
                  if (addedMemberFamilyId) {
                    familyMemberId = addedMemberFamilyId;
                    // console.log(`✅ Found family_member_id from ADD response: ${familyMemberId}`);
                  } else {
                    ...
                    console.warn(`❌ ADD response family_member_id:`, addedMemberFamilyId || 'N/A');
                  }
                }
              }
          ...
          }, 10000); // 10 detik delay untuk mendapat fresh member_id setelah ADD (sama seperti handler_bulanan.js)
        }
      ...
      console.warn(`⚠️ [SOCKET HANG UP] Possible server still processing - API ADD execution time: ${addExecutionTime}s`);
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            console.warn(`⚠️ Edit kuota timeout case gagal untuk ${nomor_hp} SLOT ${nomor_slot}:`);
            console.warn(`   - Error: ${editErr.message}`);
            console.warn(`   - Member ID: ${familyMemberId || 'NOT_FOUND'}`);
            console.warn(`   - Allocation: ${kuberInBytes || 'NOT_SET'} bytes`);
          }
        }, 10000);
