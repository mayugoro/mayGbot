...
const checkSlotKosong = async (chatId) => {
  ...
  try {
    ...
    // STRICT MODE: HANYA gunakan slot dengan sisa-add === 1
    // TIDAK ADA fallback ke sisa-add lain (0, 2, 3)
    const kosongSisa1 = slotList.filter(s => (!s.nomor || s.nomor === "") && s["sisa-add"] === 1);
    
    // HAPUS fallback logic - hanya gunakan kosongSisa1
    const kosong = kosongSisa1; // STRICT: Hanya slot dengan sisa-add === 1

    // Debug urutan slot yang dipilih
    // console.log('STRICT MODE - Slot kosong dengan sisa-add === 1:', kosong.map(s => ({ slot: s["slot-ke"], sisa: s["sisa-add"] })));

    if (!kosong.length) {
      // RELEASE LOCK karena tidak ada slot dengan sisa-add === 1
      releasePengelolaLock(nomor_hp, userId, 'no_slots_with_sisa_add_1');
      stateBekasan.delete(chatId);

      // Pesan khusus untuk STRICT MODE
      let teksKosong = '❌ <b>Tidak ada slot dengan sisa-add: 1</b>\n\n';
      
      // Hitung total slot kosong yang tersedia tapi tidak memenuhi kriteria
      const totalSlotKosong = slotList.filter(s => (!s.nomor || s.nomor === "")).length;
      
      if (totalSlotKosong > 0) {
        teksKosong += `Ditemukan ${totalSlotKosong} slot kosong, tetapi tidak ada yang memiliki sisa-add: 1.\n\n`;
        teksKosong += 'STRICT MODE: Hanya slot dengan sisa-add: 1 yang diperbolehkan.\n\n';
        teksKosong += 'Silakan coba lagi nanti.';
      } else {
        teksKosong += 'Tidak ada slot kosong yang tersedia.\n\n';
        if ((!slotList || slotList.length === 0)) {
          teksKosong += '⚠️ <i>Data tidak tersedia setelah 2x hit API</i>\n\n';
        }
        teksKosong += 'Silahkan klik lagi ✅ LANJUT BELI!';
      }
      ...
      if (loadingMessageId) {
        try {
          ...
          await safeDeleteMessage(global.bot, chatId, loadingMessageId, 'no slots with sisa-add 1');
        } catch (e) {
          // Ignore send/delete error
        }
      } else {
