...
module.exports = (bot) => {
    ...
    if (data === 'set_pesan_global') {
      ...
      try {
        ...
        adminState.set(chatId, { 
          mode: 'set_pesan_global', 
          menuMessageId: msgId 
        });
        
        const inputMsg = await bot.sendMessage(chatId, 
          `🌍 <b>SET PESAN TOLAK GLOBAL</b>\n\n` +
          `💬 <b>Pesan saat ini:</b>\n${currentPesan}\n\n` +
          `💡 Masukkan pesan baru yang akan muncul saat user tidak punya saldo cukup untuk akses global:\n\n` +
          `📝 <b>Catatan:</b>\n` +
          `• Gunakan \\n untuk baris baru\n` +
  ...
  bot.on('message', async (msg) => {
    ...
    if (!state || state.mode !== 'set_pesan_global') return;
    ...
    try {
      await setKonfigurasi('pesan_tolak_global', pesan);

      const successMessage = `✅ Pesan tolak global berhasil diubah menjadi:\n${pesan}`;
