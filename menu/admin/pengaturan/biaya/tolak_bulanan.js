...
module.exports = (bot) => {
    ...
    if (data === 'set_pesan_bulanan') {
      ...
      try {
        ...
        adminState.set(chatId, { 
          mode: 'set_pesan_bulanan', 
          menuMessageId: msgId 
        });
        
        const inputMsg = await bot.sendMessage(chatId, 
          `📋 <b>SET PESAN TOLAK BULANAN</b>\n\n` +
          `💬 <b>Pesan saat ini:</b>\n${currentPesan}\n\n` +
          `💡 Masukkan pesan baru yang akan muncul saat user tidak punya saldo cukup untuk akses bulanan:\n\n` +
          `📝 <b>Catatan:</b>\n` +
          `• Gunakan \\n untuk baris baru\n` +
  ...
  bot.on('message', async (msg) => {
    ...
    if (!state || state.mode !== 'set_pesan_bulanan') return;
    ...
    try {
      await setKonfigurasi('pesan_tolak_bulanan', pesan);

      const successMessage = `✅ Pesan tolak bulanan berhasil diubah menjadi:\n${pesan}`;
