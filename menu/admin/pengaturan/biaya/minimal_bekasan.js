...
module.exports = (bot) => {
    ...
    if (data === 'set_min_saldo_bekasan') {
      ...
      try {
        ...
        adminState.set(chatId, { 
          mode: 'set_min_saldo_bekasan', 
          menuMessageId: msgId 
        });
        
        const inputMsg = await bot.sendMessage(chatId, 
          `💰 <b>SET MIN SALDO BEKASAN</b>\n\n` +
          `💳 <b>Minimal saldo saat ini:</b> Rp. ${currentSaldo.toLocaleString('id-ID')}\n\n` +
          `💡 Masukkan minimal saldo baru untuk akses menu bekasan:\n\n` +
          `📝 <b>Catatan:</b>\n` +
  ...
  bot.on('message', async (msg) => {
    ...
    if (!state || state.mode !== 'set_min_saldo_bekasan') return;
    ...
    try {
      ...
      const successMessage = `✅ Minimal saldo bekasan berhasil diubah menjadi Rp. ${nilai.toLocaleString('id-ID')}`;
      ...
      const errorMessage = `❌ Gagal mengubah minimal saldo bekasan: ${e.message}`;
