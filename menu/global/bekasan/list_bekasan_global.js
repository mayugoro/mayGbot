  ...
  bot.on('callback_query', async (query) => {
    ...
    try {
      ...
      if (data === 'cek_stok_bekasan_global') {
        ...
        // Ambil minimal saldo dari database (gunakan config bekasan)
        const minSaldo = await getKonfigurasi('min_saldo_bekasan');
        const minSaldoValue = minSaldo ? parseInt(minSaldo) : 50000;

        // Pop-up alert untuk penolakan akses
        if (saldo < minSaldoValue) {
          const pesanTolak = await getKonfigurasi('pesan_tolak_bekasan') || 'Saldo tidak cukup untuk akses menu bekasan global\n\n⏤͟͟ᴍᴀʏᴜɢᴏʀᴏ';
